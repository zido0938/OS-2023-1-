...
void *check_rows(void *arg){
    for(int i=0; i<9; i++){
        ...
        valid[0][i] = 1;
        for(int j=0; j<9; j++){
            cnt[sudoku[i][j]]++; /* 스도쿠의 각 숫자가 몇 번 나왔는지 cnt로 저장 */
        }
        for(int j=1; j<=9; j++){ 
            if(cnt[j] != 1){ /* 1~9 숫자중에 중복된 숫자가 나타나면 */
                valid[0][i] = 0; /* 비정상으로 분류 */
                break;
            } 
        }
    }
...
void *check_columns(void *arg){
    for(int i=0; i<9; i++){ 
        ...
        valid[1][i] = 1;
        for(int j=0; j<9; j++){
            cnt[sudoku[j][i]]++; /* 스도쿠의 각 숫자가 몇 번 나왔는지 cnt로 저장 */
        }
        for(int j=1; j<=9; j++){
            if(cnt[j] != 1){ /* 1~9 숫자중에 중복된 숫자가 나타나면 */
                valid[1][i] = 0; /* 비정상으로 분류 */
                break;
            }
        }
    }
...
void *check_subgrid(void *arg){
    int id = *(int *)arg; /* subgrid 의 id */
    int i = 3*(id/3); /* subgrid 의 시작 행 */
    int j = 3*(id%3); /* subgrid 의 시작 열 */
    int cnt[10] = {0,};
    ...
    for(int n=1; n<=9; n++){
        if(cnt[n] != 1){ /* 1~9 숫자중에 중복된 숫자가 나타나면*/
            valid[2][id] = 0; /* 비정상으로 분류 */
            break;
        }
    }
    pthread_exit(NULL); /* 스레드 종료 */
